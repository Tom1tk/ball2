...
> AGENT-NOTE: Lock-on *acquisition + validity + range/angle gating + break conditions + commit-on-charge* are pure logic → `Ball2.Core` (`LockOnResolver`), Lane A, fully unit-testable. The reticle/icon rendering and the boost impulse toward the locked target are thin `Ball2.Gameplay` adapters, Lane B. Tuning numbers (exact range, tracking strength, dodge tolerance) are Q12-class — parameterise them, don't hard-code.
...
- **B2-010** · Lane A · *Lock-on resolver (acquisition/gating/break/commit per §3.3).* depends_on: B2-003. **UNBLOCKED** (Q13 settled).
